#pragma once
const DWORD KEY_UP = 0x00000001;	// 0000 0000 0000 0000 0000 0000 0000 0001
const DWORD KEY_DOWN = 0x00000002;	// 0000 0000 0000 0000 0000 0000 0000 0010
const DWORD KEY_LEFT = 0x00000004;	// 0000 0000 0000 0000 0000 0000 0000 0100
const DWORD KEY_RIGHT = 0x00000008;	// 0000 0000 0000 0000 0000 0000 0000 1000

const DWORD KEY_RETURN = 0x00000010;	// 0000 0000 0000 0000 0000 0000 0001 0000
const DWORD KEY_SPACE = 0x00000020;		// 0000 0000 0000 0000 0000 0000 0010 0000

const DWORD KEY_LBUTTON = 0x00000040;	// 0000 0000 0000 0000 0000 0000 0100 0000
const DWORD KEY_RBUTTON = 0x00000080;	// 0000 0000 0000 0000 0000 0000 1000 0000

const DWORD KEY_A = 0x00000100; 
const DWORD KEY_D = 0x00000200; 
const DWORD KEY_S = 0x00000400;
const DWORD KEY_V = 0x00000800;

const DWORD KEY_Q =		0x00001000;
const DWORD KEY_W =		0x00002000;
const DWORD KEY_ONE =	0x00004000;
const DWORD KEY_TWO =	0x00008000;
const DWORD KEY_E =		0x00010000;
const DWORD KEY_R =		0x00020000;
const DWORD KEY_T =		0x00040000;
const DWORD KEY_P =		0x00080000;
const DWORD KEY_K = 0x00100000;
const DWORD KEY_M = 0x00200000;
class CKeyMgr
{
	DEFINE_SINGLETONE(CKeyMgr)
private:
	CKeyMgr();
	~CKeyMgr();

public:
	void UpdateKey();

public:
	bool KeyDown(DWORD dwCurKey);
	bool KeyUp(DWORD dwCurKey);
	bool KeyPressing(DWORD dwCurKey);
	bool KeyCombined(DWORD iKeyPressing, DWORD iKeyDown);

	
private:
	DWORD m_dwCurKey;
	DWORD m_dwKeyDown;
	DWORD m_dwKeyUp;
	
};

