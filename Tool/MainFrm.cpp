...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
	...
	// 열의 너비를 설정하는 함수.
	// SetColumnInfo(어디 열이냐?, 너비, 허용 가능한 최소 너비)
	m_MainSplitt.SetColumnInfo(0, 300, 0);

	m_SecondSplitt.CreateStatic(&m_MainSplitt, 2, 1,
		WS_CHILD | WS_VISIBLE, m_MainSplitt.IdFromRowCol(0, 0));
	m_SecondSplitt.CreateView(0, 0, RUNTIME_CLASS(CMiniView), CSize(300, WINCY / 2), pContext);
	m_SecondSplitt.CreateView(1, 0, RUNTIME_CLASS(CMyForm), CSize(300, WINCY / 2), pContext);
